...
<html lang="en">
    ...
    <body>
        ...
        <div class="container-fluid">
            ...
            $orders = [];
            $result = $link->query("SELECT id FROM transaction ORDER BY id ASC");
            while ($record = $result->fetch()){
                if($_SESSION['aut_user']->checkRoleNoRedirrect(new Role('name', 'Store-seller'))){
                    $orders[] = new Transaction($record['id']);
                }
                else{
                    $checkOrder = new Transaction($record['id']);
                    if($checkOrder->getRegisteredFor()->getId() == $_SESSION['aut_user']->getId()){
                        $orders[] = $checkOrder;
                    }
                    else{
                        $checkOrder = null;
                        ...
                        <table class="table" id="order-list">
                            ...
                            <tbody>
                                <?php
                                    ...
                                    echo '<td>';
                                    #link to the order
                                    echo '<a href="order.php?id=' . $order->getId() . '">' . $order->getId() . '</a>';
                                    echo '</td>';
                                    echo '<td>';
                                    #link to the order
                                    echo '<a href="order.php?id=' . $order->getId() . '">' . $order->getRegisteredFor()->getFullName() . '</a>';
                                    echo '</td>';
                                    echo '<td>';
                                    echo $order->getBookInstance()->getName();
                                    echo '</td>';
                                    echo '<td>';
                                    echo $order->getFormattedRegistered();
                                    echo '</td>';  
                                    echo '<td>';
                                    $order->viewPDF();
                                    echo '</td>';
