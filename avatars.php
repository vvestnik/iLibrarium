...
<html lang="en">
    <head>
        ...
        <title>Avatar gallery - iLibrarium</title>
        ...
        if(isset($_GET['user_id'])){
            if(!empty($_GET['user_id']) || $_GET['user_id'] == 0){
                ...
                $_SESSION['login_message'] = 'No id sent, to change user avatar';
                header('Location: index.php');
                exit();
            }
        ?>
    ...
    <body>
                        ...
                        ?>                                        
                            <li class="dropdown">
                                ...
                                <ul class="dropdown-menu" aria-labelledby="about-us">
                                    <li><a href="user.php?id=<?php echo $my_id; ?>">Settings</a></li>
        ...
        <div class="container-fluid">
            ...
            <!-- Center Column -->
            <?php
            #get data about avatar
            $connection = new Connection();
            $link = $connection->connect();
            $result = $link->query("SELECT id FROM avatar");
            $avatars = [];
            while($record = $result->fetch()){
                $avatars[] = new Avatar($record['id']);
            }
            ?>
            <div class="col-sm-6">
                <h2>Avatars gallery</h2>
                <div class="row">
                    <?php
                    foreach($avatars as $avatar){
                        echo '<div class="col-xs-2">';
                        echo '<a href="big_avatar.php?avatar_id=' . $avatar->getId() . '&';                         
                        echo 'user_id=' . $my_id;   
                        echo '" class="thumbnail">';
                        echo '<img src="avatars/' . $avatar->getFilename() . '" alt="avatar_' . $avatar->getId() . '">';
                        echo '</a>';
                        echo '</div>';
                    }
                    ?>
